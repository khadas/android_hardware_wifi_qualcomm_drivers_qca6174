...
int UartRead(struct uart_t* pUartInfo, char *buf, int len)
{
	...
	ncopy = 0;
	nread = UARTRXBUFFER - pUartInfo->rx_len; // THE Data lenth already read
	nread = read(pUartInfo->fd, &(pUartInfo->rx_buffer[pUartInfo->rx_len]), nread);

#ifdef UART_DEBUG 
	// TODO: ADD debug log here to check what QDRAT has sent to me.
	// TODO: Beacause i don't know why check '\n' in received data.
/* 
*	BE NOTE:printf or fprintf(sdtout,"",...,) are not good choise to debug, Becasue they sendout the characters to screen just when it receive a '\n'.
*	It is not a realtime printout.
*	We recommend to use fprintf(stderr,"",...)	
*/
	{// read data
		int i = 0, j = 0;
		for (i = 0; i < nread; i++) {
			fprintf(stderr, "%c", (pUartInfo->rx_buffer[pUartInfo->rx_len + i]));
			if (++j % 16 == 0)
				fprintf(stderr, "\n");
		}
	}
/*
	{// write back
		...
		write_cnt = UartWrite(pUartInfo, (char *)&pUartInfo->rx_buffer[pUartInfo->rx_len], nread);
		return 0;
	}
*/
#endif
	...
	if(nread <= 0) {
		...
		if (WHICH_MODE == NO_BLOCK_MODE) {	// noblock mode
			if (nread == -1) {			
				if (errno == EAGAIN) 			// No data in UART to read, Kernel will set errno == EAGAIN to anounce that
					return 0;	   
			} 
			return nread;
		} else {								// block mode
	...
	pUartInfo->rx_len += nread; // add the new date lenth received just now
	
	// check to see if we have enough stored data to return a message
	for(il = 0; il < pUartInfo->rx_len; il++) {// terminationChar
		// TODO: Does it mean the dead line of the command receive from QDART ?? 
	//	if ((pUartInfo->rx_buffer[il] == '\n') || (pUartInfo->rx_buffer[il] == '~')) {
		if ((pUartInfo->rx_buffer[il] == DIAG_TERM_CHAR)) {
			il++;
			if(len < il) {
				ncopy = len - 1;
			} else {
			...
			memcpy(buf, pUartInfo->rx_buffer, ncopy); // copy data to user buffer
			buf[ncopy] = 0;
			memcpy(pUartInfo->rx_buffer, &pUartInfo->rx_buffer[il], pUartInfo->rx_len - il);// compress the internal data
			pUartInfo->rx_len -= il;
	//		printf("%s --> find the terminal char 0x%x at %d\n",  __func__, pUartInfo->rx_buffer[il -1], il - 1); // DEBUG USE
			return ncopy;
		}
	}
    	// nart doesn't send terminationChar
    	if (pUartInfo->rx_len > 0 && il == pUartInfo->rx_len) {// there is no terminate char sent by QDART
      		ncopy = pUartInfo->rx_len;
		if (ncopy < MBUFFER) {
			memcpy(buf, pUartInfo->rx_buffer, ncopy);
	    		buf[ncopy] = 0;
		}else {		
			//1TODO: What should we do??  
