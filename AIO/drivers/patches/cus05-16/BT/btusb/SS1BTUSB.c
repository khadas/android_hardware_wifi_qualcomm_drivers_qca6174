...
/*#define SS1BTUSB_CLASS_NAME             SS1BTUSB0*/
...
static void CopyURBMemoryToLocalReadMemory(struct urb *URBPtr, USBBufferInformation_t *BufferInfo, uint8_t PktType);
...
static void CopyURBMemoryToLocalReadMemory(struct urb *URBPtr, USBBufferInformation_t *BufferInfo, uint8_t PktType)
{
   ...
   if(URBPtr)
   {
      ...
      if((LengthOfData <= ((BufferInfo->CircleBufferSize - BufferInfo->CircleBufferBytesAvailable) + 1)))
      {
         ...
         if((BufferInfo->CircleBufferWriteIndex + LengthOfData) > BufferInfo->CircleBufferSize)
         {
            ...
            printk(KERN_ERR "%s: HCI Event Buffer is wrapping up!\n", __func__);

            /* Use the 1st byte for indicating packet type */
            memcpy(&(BufferInfo->CircleBuffer[BufferInfo->CircleBufferWriteIndex]), &PktType, sizeof(uint8_t));

            /* Copy one byte less, since we have used one byte for saving the packet type */
            memcpy(&(BufferInfo->CircleBuffer[BufferInfo->CircleBufferWriteIndex+1]), TempPtr, ((BufferInfo->CircleBufferSize - BufferInfo->CircleBufferWriteIndex) - 1));

            /* Now Copy the data starting at the beginning of the       */
            /* Buffer.                                                  */
            memcpy(&(BufferInfo->CircleBuffer[0]), &TempPtr[(BufferInfo->CircleBufferSize - BufferInfo->CircleBufferWriteIndex) - 1], (LengthOfData - ((BufferInfo->CircleBufferSize - BufferInfo->CircleBufferWriteIndex) + 1)));
         }
         else
         {
            ...
            /* the Buffer.                                              */
            memcpy(&(BufferInfo->CircleBuffer[BufferInfo->CircleBufferWriteIndex]), &PktType, sizeof(uint8_t));
            memcpy(&(BufferInfo->CircleBuffer[BufferInfo->CircleBufferWriteIndex+1]), TempPtr, LengthOfData);
         }

         /* Increment the Data Write Pointer and the amount of data put */
         /* into the Input Buffer.                                      */
         /* Increment by 1 to accomodate the packet type */
         BufferInfo->CircleBufferBytesAvailable += LengthOfData + 1;
         BufferInfo->CircleBufferWriteIndex     += LengthOfData + 1;
...
static void CloseDevice(USBDeviceInformation_t *USBDeviceInformationPtr)
{
   ...
   DEBUGPRINT(KERN_ERR "%s: #####<CLEANUP>##### USB Device Cleanup Inprogress...\n", __FUNCTION__);

   /* First check for pending SCO data write urbs in process.           */
   DEBUGPRINT(KERN_ERR "%s Pending SCO writes: %d\n", __FUNCTION__,
       atomic_read(&(USBDeviceInformationPtr->SCOBufferInformation.OutstandingWrites)));
   ...
   DEBUGPRINT(KERN_ERR "%s Pending ACL writes: %d\n", __FUNCTION__,
       atomic_read(&(USBDeviceInformationPtr->ACLBufferInformation.OutstandingWrites)));
   ...
   if(atomic_read(&(USBDeviceInformationPtr->ACLBufferInformation.OutstandingWrites)))
   {
      for(Index=0;Index<MAX_WRITE_BUFFERS;Index++)
      {
         ...
         if(!IsItemInList(&(USBDeviceInformationPtr->ACLBufferInformation.FreeURBBufferList), &(USBDeviceInformationPtr->ACLBufferInformation.URBBufferInfo[Index].ListEntry)))
         {
            ...
            DEBUGPRINT(KERN_ERR "\tKilling ACL urb %p\n", USBDeviceInformationPtr->ACLBufferInformation.URBBufferInfo[Index].URB);
         }
      }
   }

   /* Last check for pending HCI data write urbs in process.            */
   DEBUGPRINT(KERN_ERR "%s Pending HCI data writes: %d\n", __FUNCTION__,
       atomic_read(&(USBDeviceInformationPtr->HCIBufferInformation.OutstandingWrites)));
   ...
   if(atomic_read(&(USBDeviceInformationPtr->HCIBufferInformation.OutstandingWrites)))
   {
      for(Index=0;Index<MAX_WRITE_BUFFERS;Index++)
      {
         ...
         if(!IsItemInList(&(USBDeviceInformationPtr->HCIBufferInformation.FreeURBBufferList), &(USBDeviceInformationPtr->HCIBufferInformation.URBBufferInfo[Index].ListEntry)))
         {
            ...
            DEBUGPRINT(KERN_ERR "\tKilling HCI urb %p\n", USBDeviceInformationPtr->HCIBufferInformation.URBBufferInfo[Index].URB);
         }
      }
   }

   DEBUGPRINT(KERN_ERR "%s : Stop reading URB's\n", __FUNCTION__);
   ...
   DEBUGPRINT(KERN_ERR "%s: #####<CLEANUP>##### USB Device Cleanup Complete \n", __FUNCTION__);
}
   ...
   if((URBPtr) && (URBPtr->context))
   {
      ...
      if(USBBufferInformationPtr)
      {
         ...
         if((URBPtr->status) || (!URBPtr->actual_length))
         {
            ...
            DEBUGPRINT(KERN_ERR ": %s: Write Successful: URB Status = %d, Count = %d\n", __FUNCTION__, URBPtr->status, URBPtr->actual_length);

         /* Obtain the write SpinLock.                                  */
         DEBUGPRINT(KERN_INFO "%s: Waiting for Write Information SpinLock\n", __FUNCTION__);
         ...
         DEBUGPRINT(KERN_INFO "%s: Set Write Wait Event\n", __FUNCTION__);
         wake_up(&(USBBufferInformationPtr->WriteWaitEvent));

         DEBUGPRINT(KERN_INFO "%s:  URB %p, buffer %p, BufferInfo %p processed, OutstandingWrites %d\n", __FUNCTION__, URBPtr, URBPtr->transfer_buffer, URBPtr->context, atomic_read(&(USBBufferInformationPtr->OutstandingWrites)));

         /* Release the SpinLock.                                       */
         DEBUGPRINT(KERN_INFO "%s: Releasing Write Information SpinLock\n", __FUNCTION__);

         spin_unlock(&(USBBufferInformationPtr->WriteInformationSpinLock));
      } else
            DEBUGPRINT(KERN_ERR ": %s: Failed to retrieve USBBufferInformationPtr from the URB Context\n", __FUNCTION__);
   } else
            DEBUGPRINT(KERN_ERR ": %s: URB Pointer looks INVALID\n", __FUNCTION__);
}
...
static int ath3k_load_patch(struct usb_device *udev)
{
	...
	ret = android_request_firmware(&firmware, filename, &udev->dev);
	if (ret < 0) {
...
static int USBProbe(struct usb_interface *InterfacePtr, const struct usb_device_id *DeviceID)
{
   ...
   printk(KERN_ERR "%s: BT-USB Probe: Check if device connected is ROME-USB\n", __func__);
   printk(KERN_ERR "%s: BTUSB_IOCTL_MAGIC is %x\n", __func__, BTUSB_IOCTL_MAGIC);
   ...
   if((INode) && (FilePointer))
   {
      ...
      if((USBDeviceInformationPtr) && (!USBDeviceInformationPtr->BeingRemoved))
      {
         ...
         if(!USBDeviceInformationPtr->OpenedByUser)
         {
            ...
            DEBUGPRINT(KERN_INFO "%s : Possible Urb Status ENOENT -%d ECONNRESET -%d EINPROGRESS -%d EPIPE -%d EREMOTEIO -%d ENODEV -%d EINVAL -%d\n", __FUNCTION__,
...
static int USBClose(struct inode *INode, struct file *FilePointer)
{
   ...
   //printk(KERN_INFO "%s : Enter INode %p FilePointer %p \n", __FUNCTION__, INode, FilePointer);
...
static int CopyUSBPacketFromUser(unsigned long IoctlParameter, USBPacketInformation_t *USBPacketInformation, uint8_t Compat)
{
   ...
   if((IoctlParameter) && (USBPacketInformation))
   {
      ...
      {
         if((ret_val = copy_from_user(&USBPacketInformation->PacketType, &USBPacketInformationPtr->PacketType, sizeof(USBPacketInformation->PacketType))) == 0)
         {
            if((ret_val = copy_from_user(&USBPacketInformation->BufferSize, &USBPacketInformationPtr->BufferSize, sizeof(USBPacketInformation->BufferSize))) == 0)
            {
...
static int WriteDataIoctl(USBDeviceInformation_t *USBDeviceInformationPtr, unsigned long Parameter, uint8_t Compat)
{
   ...
   if(Parameter)
   {
      ...
      BT_ERR("%s: sizeof(USBPacketInformation_t) is %ld", __func__, sizeof(USBPacketInformation_t));
      if(CopyUSBPacketFromUser(Parameter, &USBPacketInformation, Compat) == 0)
      {
         switch(USBPacketInformation.PacketType)
         {
            case BTUSB_PACKET_TYPE_COMMAND:
               //BT_ERR("%s: BTUSB_PACKET_TYPE_COMMAND: OPCODE: 0x%x", __func__, ((USBPacketInformation.Buffer[1] << 8) | USBPacketInformation.Buffer[0]));
               ...
               while(USBPacketInformation.BufferSize)
               {
                  ...
                  if(URBBufferInfo)
                  {
                     ...
                     if((URBBufferInfo->URB) && (URBBufferInfo->URBBuffer))
                     {
                        ...
                        if(copy_from_user(URBBufferInfo->URBBuffer, USBPacketInformation.Buffer, BytesToSend) == 0)
                        {
                           ...
                           DEBUGPRINT(KERN_ERR "%s: Issuing an async transfer request for EP0OUT\n", __FUNCTION__);
                        ...
                        {
                           DEBUGPRINT(KERN_ERR "%s: copy_from_user() failure!! Failed to copy the command to be sent\n", __FUNCTION__);
                     ...
                     {
                        DEBUGPRINT(KERN_ERR "%s: No URB and/or URBBuffer available!!!\n", __FUNCTION__);
                  ...
                  {
                     DEBUGPRINT(KERN_ERR "%s: No free URB available in the FreeURBBufferList!!!\n", __FUNCTION__);
...
static void StopHCIEventACL(USBDeviceInformation_t *USBDeviceInformationPtr)
{
   ...
   if(USBDeviceInformationPtr)
   {
      ...
      DEBUGPRINT(KERN_ERR "%s USB Core stopped reading ACL Data\n", __FUNCTION__);

      usb_kill_urb(USBDeviceInformationPtr->HCIBufferInformation.ReadURBHandle);
      DEBUGPRINT(KERN_ERR "%s USB Core stopped reading HCI Event\n", __FUNCTION__);
   } else
      DEBUGPRINT(KERN_ERR "%s USBDeviceInformationPtr unavailable for cleanup!!!\n", __FUNCTION__);
}
...
int BytesAvailableToRead(USBBufferInformation_t *USBBufferInformationPtr, int PktType, int Max)
{
   int ReadLen = 0;
   int Offset = 0;
   int PayloadLen = 0;
   ...
   printk(KERN_ERR "%s ++, PktType = %d Max = %d", __func__, PktType, Max);
   if (USBBufferInformationPtr) {
      BufPtr = &USBBufferInformationPtr->CircleBuffer[USBBufferInformationPtr->CircleBufferReadIndex];

      if (USBBufferInformationPtr->CircleBufferBytesAvailable <= Max) {
         printk(KERN_ERR "%s return bytes 1 = %d ", __func__, USBBufferInformationPtr->CircleBufferBytesAvailable);
         return USBBufferInformationPtr->CircleBufferBytesAvailable;
      }
AGAIN:
   switch (PktType)
      {
         case BTUSB_PACKET_TYPE_ACL:
            Offset = BTUSB_HCI_ACL_HDR_LEN_OFFSET;
            PayloadLen = (BufPtr[Offset] | (BufPtr[Offset + 1] << 8));
            PktLen = BTUSB_PKT_IND_LENGTH + BTUSB_HCI_ACL_HDR_LENGTH + PayloadLen;
            break;
         case BTUSB_PACKET_TYPE_EVENT:
            Offset = BTUSB_HCI_EVT_HDR_LENGTH;
            PayloadLen = BufPtr[Offset];
            PktLen = BTUSB_PKT_IND_LENGTH + BTUSB_HCI_EVT_HDR_LENGTH + PayloadLen;
            break;
         case BTUSB_PACKET_TYPE_SCO:
            //TODO:
            break;
   ...
   printk(KERN_ERR "%s PktLen= %d", __func__, PktLen);

      if ((ReadLen + PktLen) <= Max) {
         ReadLen += PktLen;
         BufPtr = BufPtr + PktLen;
         goto AGAIN;
      }
   }

   printk(KERN_ERR "%s -- returns bytes 2= %d", __func__, ReadLen);
   return ReadLen;
}
               ...
               if(Parameter)
               {
                  ...
                  if(copy_from_user(&USBPacketInformation, (USBPacketInformation_t *)Parameter, sizeof(USBPacketInformation_t)) == 0)
                  {
                     ...
                     if(USBBufferInformationPtr)
                     {
                        ...
                        ret_val = BytesAvailableToRead(USBBufferInformationPtr, USBPacketInformation.PacketType, USBPacketInformation.BufferSize);
                        ...
                        if (USBPacketInformation.Buffer[0] == 0xe)
				DEBUGPRINT(KERN_ERR "%s : Received HCI CMD-COMPLETE Event for OPCODE: 0x%x with status: %d\n", __FUNCTION__, ((USBPacketInformation.Buffer[4] << 8) | USBPacketInformation.Buffer[3]), USBPacketInformation.Buffer[5]);
                        else {
                            ...
                            for (i = 0; i < ((ret_val > 6)?6:ret_val); i++)
                                DEBUGPRINT(KERN_ERR "0x%2x    ", USBPacketInformation.Buffer[i]);
                        }
			DEBUGPRINT(KERN_ERR "\n");
                     }
                  }
      ...
      if((USBDeviceInformationPtr) && (!USBDeviceInformationPtr->BeingRemoved))
      {
         ...
         switch(Command)
         {
            ...
            //case BTUSB_IOCTL_WAIT_DATA32:
               ...
               break;
            case BTUSB_IOCTL_READ_PACKET:
            //case BTUSB_IOCTL_READ_PACKET32:
               DEBUGPRINT(KERN_ERR "%s : Read Data\n", __FUNCTION__);
            ...
            BT_ERR("");
               ...
               break;
            case BTUSB_IOCTL_SEND_PACKET:
            //case BTUSB_IOCTL_SEND_PACKET32:
               DEBUGPRINT(KERN_ERR "%s : Send Data\n", __FUNCTION__);
...
int __init InitializeBTUSBModule(void)
{
   ...
   printk(KERN_ERR "%s : Module initialization in-progress...\n", __FUNCTION__);
   ...
   printk(KERN_ERR "%s: Registering the BT USB driver with the USB Core & Kernel device/driver Subsystem...", __func__);
   if((ret_val = usb_register(&ContextInformation.USBDriver)) < 0)
   {
      printk(KERN_ERR "%s : Failed to register USB driver ret_val in %d\n", __FUNCTION__ , ret_val);
   }

   printk(KERN_ERR "%s : Exiting module load...\n", __FUNCTION__);
